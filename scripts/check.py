...
def readme_sanity() -> bool:
    ...
    problems = []
    if 'python -m pip install "compile-code @ git+https://github.com/Cranot/compile-code.git@v0.2.2"' not in text:
        problems.append("install command missing")
    if 'python -m pip install "compile-code==0.2.2"' not in text:
...
def release_sanity() -> bool:
    ...
    second_patch_dates = re.findall(r"(?m)^## 0\.2\.2 - (\d{4}-\d{2}-\d{2})$", changelog)
    if second_patch_dates != ["2026-08-16"]:
        problems.append(f"CHANGELOG.md: expected one 0.2.2 release heading dated 2026-08-16, got {second_patch_dates}")
