...
RELEASE_CONSTRAINTS_NAME = "shared-constraints.txt"
# release.yml installs three of these locks through one `pip install
# --require-hashes`, so one distribution resolved at two versions is not a
# cosmetic inconsistency: it is a ResolutionImpossible that only the combined
# install can see. Every graph is compiled against the shared constraints file
# so agreement holds by construction, and this list names the locks whose rows
# must be covered by it.
COMBINED_INSTALL_LOCKS = (
...
def _parse_release_input(path: Path, *, kind: str = "input") -> dict[str, str]:
    label = f"release {kind} {path.name}"
...
def _parse_release_lock(path: Path, *, input_name: str) -> dict[str, str]:
    ...
    # The header must keep describing the command that actually produced the
    # file. Every graph is now compiled against release/shared-constraints.txt,
    # so the constraint is part of the recorded command rather than something a
    # hand edit could hide: regenerating without it fails this assertion.
    expected_command = (
        f"#    uv pip compile release/{input_name} --constraint release/{RELEASE_CONSTRAINTS_NAME} "
        f"--universal --python-version 3.10 --generate-hashes "
...
def shared_constraint_failures(root: Path = ROOT) -> list[str]:
    """Report every way the release graphs could still disagree about a shared row.

    A per-lock check cannot see this class of defect: each lock is internally
    consistent while the combined `pip install --require-hashes` in release.yml
    is unsatisfiable. The shared constraints file makes agreement the resolver's
    job; these assertions prove it stayed that way.
    """
    release_dir = _validated_real_directory(root / "release", label="release lock")
    constraints = _parse_release_input(release_dir / RELEASE_CONSTRAINTS_NAME, kind="constraints")
    graphs: dict[str, dict[str, str]] = {}
    roots: dict[str, dict[str, str]] = {}
    for input_name, lock_name in LOCK_GRAPHS:
        roots[input_name] = _parse_release_input(release_dir / input_name)
        graphs[lock_name] = _parse_release_lock(release_dir / lock_name, input_name=input_name)

    failures: list[str] = []
    resolved: dict[str, dict[str, set[str]]] = {}
    for lock_name, locked in graphs.items():
        for name, version in locked.items():
            resolved.setdefault(name, {}).setdefault(version, set()).add(lock_name)

    for name, versions in sorted(resolved.items()):
        locks = sorted({lock for holders in versions.values() for lock in holders})
        if len(versions) > 1:
            detail = "; ".join(
                f"{version} in {', '.join(sorted(holders))}" for version, holders in sorted(versions.items())
            )
            consequence = (
                f" — the combined release.yml install of {', '.join(COMBINED_INSTALL_LOCKS)} is unsatisfiable"
                if len(set(locks) & set(COMBINED_INSTALL_LOCKS)) > 1
                else ""
            )
            failures.append(f"{name} is resolved at two versions across the release locks: {detail}{consequence}")
            continue
        version = next(iter(versions))
        if len(locks) < 2:
            if name in constraints:
                failures.append(
                    f"{RELEASE_CONSTRAINTS_NAME} pins {name}=={constraints[name]}, "
                    f"which only {locks[0]} resolves; the file must describe shared rows only"
                )
            continue
        if name not in constraints:
            failures.append(
                f"{name}=={version} is shared by {', '.join(locks)} but is absent from {RELEASE_CONSTRAINTS_NAME}"
            )
        elif constraints[name] != version:
            failures.append(
                f"{RELEASE_CONSTRAINTS_NAME} pins {name}=={constraints[name]} "
                f"but {', '.join(locks)} resolved {name}=={version}"
            )

    for name in sorted(set(constraints) - set(resolved)):
        failures.append(f"{RELEASE_CONSTRAINTS_NAME} pins {name}=={constraints[name]}, which no release lock resolves")

    for input_name, declared in sorted(roots.items()):
        for name, version in sorted(declared.items()):
            if name in constraints and constraints[name] != version:
                failures.append(
                    f"release input {input_name} declares {name}=={version} "
                    f"but {RELEASE_CONSTRAINTS_NAME} pins {name}=={constraints[name]}"
                )
...
def locked_requirement_queries(
    ...
    shared_failures = shared_constraint_failures(root)
    _require(not shared_failures, f"release locks disagree about a shared distribution: {'; '.join(shared_failures)}")
    _require(
...
def audit_repository(root: Path = ROOT) -> list[str]:
    ...
    required_fragments = (
        ...
        "needs: [build, require_green_ci, prepublish, github_release_preflight, github_release_draft_verify]",
